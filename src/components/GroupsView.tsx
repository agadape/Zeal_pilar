...
import { Users, UserCheck, Plus, Edit2, Trash2, X, Check, Shield } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Small Groups & Leaders</h1>
          <p className="text-sm text-slate-400">Atur kelompok kecil (PDG Brother/Sister), tetapkan Pemimpin, dan mapping anggota kelompok.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {groups.map(g => (
          <div key={g.id} className="glass-panel glass-panel-hover p-6 rounded-2xl space-y-4 relative flex flex-col justify-between border border-white/10">
            ...
            <div className="space-y-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">{g.group_name}</h3>
            ...
            <div className="pt-4 border-t border-white/10 flex items-center justify-between">
              <button
                ...
                className="w-full py-2 px-3 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-semibold flex items-center justify-center space-x-2 transition-colors"
              >
      ...
      {isGroupModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass-panel w-full max-w-md rounded-2xl p-6 border border-white/20 space-y-6 animate-fade-in bg-zinc-950">
            ...
            <form onSubmit={handleGroupSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase mb-1">Nama Small Group *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase mb-1">Tugaskan Pemimpin Group (Leader)</label>
                <select
                  value={leaderId}
      ...
      {managingMembersGroup && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass-panel w-full max-w-lg rounded-2xl p-6 border border-white/20 space-y-6 animate-fade-in bg-zinc-950 max-h-[85vh] flex flex-col">
            ...
            {loadingMembers ? (
              <div className="py-12 text-center text-slate-400 text-sm">Memuat anggota group...</div>
            ) : (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <span className="block text-[11px] text-slate-400">{p.status} {p.campus ? `• ${p.campus}` : ''}</span>
                          </div>
                        </div>
                      </div>
                    );
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/10">
              <span className="text-xs text-slate-400">{selectedMemberIds.length} Anggota terpilih</span>
              <div className="space-x-3">
                ...
                <button
                  ...
                  className="btn-glow px-5 py-2.5 rounded-xl bg-white text-black font-bold text-sm inline-flex items-center space-x-2"
                >
                  <Check className="w-4 h-4" />
